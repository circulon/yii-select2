...
class Select2 extends CInputWidget
{
    ...
    public function init()
    {
        ...
        if (isset($this->htmlOptions['select2Options'])) {
            if (array_key_exists("onTrigger", $this->htmlOptions['select2Options'])) {
                $this->onTrigger = CMap::mergeArray($this->onTrigger,
                    $this->htmlOptions['select2Options']['onTrigger']);
    ...
    private function registerScripts($id)
    {
        ...
        $lang = substr(str_replace('_', '-', strtolower(Yii::app()->language)), 0, 2);
    ...
    {
        return self::dropDownList(CHtml::activeName($model, $attribute), CHtml::value($model, $attribute), $data,
            $htmlOptions);
    }
    ...
    {
        return self::multiSelect(CHtml::activeName($model, $attribute) . '[]', CHtml::value($model, $attribute), $data,
            $htmlOptions);
    }
